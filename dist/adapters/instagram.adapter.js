...
let InstagramAdapter = InstagramAdapter_1 = class InstagramAdapter {
    ...
    parseMessagingEvent(event, entryId) {
        ...
        if (event.referral && !event.message) {
            this.logger.debug(`Instagram standalone ad referral: ad_id=${event.referral.ad_id ?? 'n/a'}, sender=${event.sender.id}`);
            return null;
        }
    ...
    parseReferral(message) {
        ...
        this.logger.debug(`Instagram ad referral parsed: sourceId=${referral.sourceId ?? 'n/a'}`);
        return referral;
    }
    /**
     ...
     */
    async fetchConversationMessages(userId, credentials) {
        try {
            const resolved = this.resolveCredentials(credentials);
            if (!resolved.accessToken) {
                throw new Error('Instagram access token not configured');
            ...
            const convUrl = `${this.igBaseUrl}/${this.apiVersion}/${igAccountId}/conversations` +
            ...
            for (const conversation of convData.data) {
                ...
                for (const msg of allMessageData) {
                    ...
                    const direction = this.determineDirectionByIdWithOverride(msg.from.id, resolved.instagramBusinessAccountId);
                    messages.push({
                        channelMessageId: msg.id,
                        direction,
                        senderName: msg.from.username ?? msg.from.id,
     ...
     */
    determineDirectionByIdWithOverride(senderId, businessAccountId) {
