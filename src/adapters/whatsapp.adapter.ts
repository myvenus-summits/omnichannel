...
export class WhatsAppAdapter implements ChannelAdapter {
   ...
   */
  private readonly clientCache = new Map<string, Twilio>();
   ...
   */
  private getOrCreateClient(accountSid: string, authToken: string): Twilio {
    const key = `${accountSid}:${authToken}`;
    const cached = this.clientCache.get(key);
    if (cached) return cached;

    if (this.clientCache.size >= WhatsAppAdapter.MAX_CACHED_CLIENTS) {
  ...
  async sendTemplateMessage(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(
        `Failed to send WhatsApp template message [contentSid=${templateId}, variables=${JSON.stringify(variables)}, twilioCode=${twilioCode}]`,
        error,
      );
      return {
        success: false,
  ...
  private parseConversationsApiPayload(
    ...
    if (twilioPayload.EventType === 'onMessageAdded') {
      ...
      return {
        ...
        message: {
          channelMessageId: this.ensureChannelMessageId(
            twilioPayload.MessageSid,
            twilioPayload.ConversationSid,
            twilioPayload.ParticipantSid,
            twilioPayload.DateCreated,
          ),
          direction,
    ...
    if (
      ...
      return {
        ...
        contactIdentifier: '',
        status: {
          messageId: this.ensureChannelMessageId(
            twilioPayload.MessageSid,
            twilioPayload.ConversationSid,
          ),
   ...
   */
  private async fetchMessagesViaMessagingApi(
    ...
    try {
      ...
      // 두 결과 merge 후 시간순 정렬
      const allMessages = [...inboundMessages, ...outboundMessages].sort(
        (a, b) => new Date(a.dateSent ?? a.dateCreated).getTime() - new Date(b.dateSent ?? b.dateCreated).getTime(),
      );

      const normalized: NormalizedMessage[] = [];

      for (const msg of allMessages) {
        const direction: MessageDirection = msg.from === customerNumber ? 'inbound' : 'outbound';
        let contentType: MessageContentType = 'text';
        let mediaUrl: string | undefined;
      ...
      this.logger.log(
        `Fetched ${normalized.length} messages via Messaging API for ${customerNumber}`,
      );
